...
La doctrine du CI-SIS peut être synthétisée par les règles suivantes (cf [Doctrine CI-SIS](https://interop.esante.gouv.fr/ig/doctrine/doctrine.html#la-doctrine-du-ci-sis))

### Les standards

#### Profils IHE : PAM

Une présentation générale du profil est disponible sur l'[IG des Normes et Standards](normes_standards_ig_annexe.md#pam).
...
| | | |
| :--- | :--- | :--- |
| [CercleSoins](StructureDefinition-CDSCercleSoins.md) | Non couvert |   |
| [MembreCercleSoins](StructureDefinition-CDSMembreCercleSoins.md) | ROL / NK1 | Pas de segment dédié ; répartition entre ROL (professionnels) et NK1 (entourage) |
| [PersonnePriseCharge](StructureDefinition-CDSPersonnePriseCharge.md) | PID | Segment d'identification patient |
| [Professionnel](StructureDefinition-CDSProfessionnel.md) | ROL | Précise la fonction (rôle) d'un intervenant médical en relation avec le patient |
| [Contact](StructureDefinition-CDSContact.md) | NK1 | Décrit les personnes à prévenir ou les personnes de confiance, mais ne permet pas de décrire les notions d'aidant et de responsable légal |
| [EntiteGeographique](StructureDefinition-CDSEntiteGeographique.md) | Non couvert | Les structures de santé ne sont identifiés dans aucun segment |
| [EntiteJuridique](StructureDefinition-CDSEntiteJuridique.md) | Non couvert | Les structures de santé ne sont identifiés dans aucun segment |
| [OrganisationInterne](StructureDefinition-CDSOrganisationInterne.md) | Non couvert | Les unités de soins ne sont identifiées dans aucun segment |
| [SituationExercice](StructureDefinition-CDSSituationExercice.md) |   |   |
| [ExerciceProfessionnel](StructureDefinition-CDSExerciceProfessionnel.md) |   |   |
...
* **Synthèse**Profil adapté à l'identité et aux déplacements du patient, mais pas au cas d'usage de gestion du cercle de soins.: - Réutiliser un standard déjà déployé dans l'écosystème réduit l'effort d'intégration côté éditeurs
...
Une présentation générale du profil est disponible sur l'[IG des Normes et Standards](normes_standards_ig_annexe.md#dctm).
...
| | | |
| :--- | :--- | :--- |
| [CercleSoins](StructureDefinition-CDSCercleSoins.md) | CareTeam |   |
| [MembreCercleSoins](StructureDefinition-CDSMembreCercleSoins.md) | CareTeam | `CareTeam.participant` |
| [PersonnePriseCharge](StructureDefinition-CDSPersonnePriseCharge.md) | Patient | `CareTeam.subject` |
| [Professionnel](StructureDefinition-CDSProfessionnel.md) | Practitioner | `CareTeam.participant.member` |
| [Contact](StructureDefinition-CDSContact.md) | RelatedPerson | `CareTeam.participant.member` |
| [EntiteGeographique](StructureDefinition-CDSEntiteGeographique.md) | Organization | `CareTeam.participant.member`/`CareTeam.managingOrganization` |
| [EntiteJuridique](StructureDefinition-CDSEntiteJuridique.md) | Organization | `CareTeam.participant.member`/`CareTeam.managingOrganization` |
| [OrganisationInterne](StructureDefinition-CDSOrganisationInterne.md) | Organization | `CareTeam.participant.member`/`CareTeam.managingOrganization` |
| [SituationExercice](StructureDefinition-CDSSituationExercice.md) |   |   |
| [ExerciceProfessionnel](StructureDefinition-CDSExerciceProfessionnel.md) |   |   |
...
Une présentation générale du standard (comprenant notamment l'outillages associé, le transport et les interactions FHIR) est disponible sur l'[IG des Normes et Standards](normes_standards_ig_annexe.md#présentation).
...
| [ExerciceProfessionnel](StructureDefinition-CDSExerciceProfessionnel.md) | [PractitionerRole](https://hl7.org/fhir/practitionerrole.html)**(NM 2)** | Détaille les rôles et spécialités qu'un praticien peut exercer dans différents services d’une organisation pendant une durée déterminée. Cette ressource est mise en correspondance avec les classes[ExerciceProfessionnel](https://ansforge.github.io/IG-modele-objets-sante/main/ig/StructureDefinition-ExerciceProfessionnel.html)et[SituationExercice](https://ansforge.github.io/IG-modele-objets-sante/main/ig/StructureDefinition-SituationExercice.html)du MOS.La ressources PractitionerRole permet de véhiculer les informations définies dans l’étude métier comme nécessaires pour décrire les professionnels de santé membres du cercle de soins |
...
* **Synthèse**La ressource CareTeam (NM2) reste aujourd’hui instable et pourrait encore être modifiée sans assurer de rétrocompatibilité avec la version actuelle. Par conséquent, il n’est pas à exclure que les spécifications d’interopérabilité de la gestion de cercles de soins subissent des modifications majeures dans le cas d’une adoption du standard FHIR.: - Montée en puissance générale de FHIR à l'international- Sessions de tests HL7 continues qui font progresser les spécifications- Écosystème d'éditeurs déjà large sur les autres ressources FHIR
  * ?: - Instabilité persistante de CareTeam → risque de refontes majeures des spécifications d'interopérabilité si adopté seul

#### openEHR

Une présentation générale du profil est disponible sur l'[IG des Normes et Standards](normes_standards_ig_annexe.md#description).
...
| | | |
| :--- | :--- | :--- |
| [CercleSoins](StructureDefinition-CDSCercleSoins.md) | GROUP | Représente le regroupement de PARTIES formé pour un objectif commun (équipe de soins). GROUP est une classe particulièrement intéressante pour le cas d’usage de la gestion du cercle de soins, en représentant le groupe « cercle de soins » d’une personne prise en charge. Ce groupe serait constitué d’acteurs (personnes ou organisations) définis par leur rôle et leur lien avec la personne prise en charge. |
| [MembreCercleSoins](StructureDefinition-CDSMembreCercleSoins.md) | PARTY | Membre du groupe, personne ou organisation |
| [PersonnePriseCharge](StructureDefinition-CDSPersonnePriseCharge.md) | PARTY | Usager pour lequel le cercle de soins est défini |
| [Professionnel](StructureDefinition-CDSProfessionnel.md) | ACTOR + ROLE + RELATIONSHIP | un ACTOR hérite de PARTY, il peut s'agir d'une personne, d'une organisation ou d'un groupe, il est capable de tenir un ROLE. Son lien avec les autres PARTIES est défini avec RELATIONSHIP |
| [Contact](StructureDefinition-CDSContact.md) | ACTOR + ROLE + RELATIONSHIP |   |
| [EntiteGeographique](StructureDefinition-CDSEntiteGeographique.md) | ACTOR + ROLE + RELATIONSHIP |   |
| [EntiteJuridique](StructureDefinition-CDSEntiteJuridique.md) | ACTOR + ROLE + RELATIONSHIP |   |
| [OrganisationInterne](StructureDefinition-CDSOrganisationInterne.md) | ACTOR + ROLE + RELATIONSHIP |   |
| [SituationExercice](StructureDefinition-CDSSituationExercice.md) |   |   |
| [ExerciceProfessionnel](StructureDefinition-CDSExerciceProfessionnel.md) |   |   |
...
Le profil **IHE DCTM**, bien que récent et non encore implémenté, couvre parfaitement le besoin d'interopérabilité de la gestion du cercle de soins tout en respectant le premier niveau de la hiérarchie doctrinale (profil IHE stable).
